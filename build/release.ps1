...
"@
    ...
    if ($existingTag) {
        throw @"
Tag $tag already exists locally. If a prior release run failed partway through,
clean up the half-finished state before retrying:
    git tag -d $tag                      # delete local tag
    git push origin :refs/tags/$tag      # delete it on Gitea (ignore error if absent)
then re-run. (Bump to a new version instead if $tag was already fully published.)
"@
    }

    # Pre-flight: a prior run may have pushed the tag and committed the version bump
    # but died before/while creating the GitHub release. Catch an already-published
    # release now with a clear message rather than failing deep in the publish phase.
    #
    # `gh release view` exits non-zero AND writes "release not found" to stderr when
    # the release is absent — which is the normal, expected state here. Under
    # EAP=Stop that stderr line becomes a terminating error, so relax EAP for just
    # this probe (same trick as Invoke-Git) and decide on $LASTEXITCODE alone.
    $oldEAP = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    try { & gh release view $tag --repo $GitHubRepo 2>&1 | Out-Null }
    finally { $ErrorActionPreference = $oldEAP }
    if ($LASTEXITCODE -eq 0) {
        throw "A GitHub release '$tag' already exists on $GitHubRepo. Delete it (``gh release delete $tag --repo $GitHubRepo``) or bump to a new version."
    }
}
