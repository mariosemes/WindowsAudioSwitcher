...
/// %APPDATA%\WindowsAudioSwitcher\logs\app-YYYYMMDD.log. The target file name is
/// recomputed on every write so a tray process left running across midnight rolls
/// onto the new day's file rather than appending to the startup day's forever.
/// </summary>
public static class Logger
{
    private static readonly object _gate = new();

    // Tracks which day the log is currently writing to so we can prune old files
    // when the day actually rolls over, not only at process start. Guarded by _gate.
    private static string _currentLogDay = DateTime.Now.ToString("yyyyMMdd");

    public static string LogDirectory { get; } = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
        "WindowsAudioSwitcher", "logs");

    /// <summary>Path to <em>today's</em> log file — recomputed per access so it rolls at midnight.</summary>
    ...
    private static void Write(string level, string message, Exception? ex)
    {
        ...
        lock (_gate)
        {
            // Roll-over check: if the calendar day changed since the last write,
            // prune aged-out files now (the static ctor's cleanup only ever runs
            // once, which never helps a process that stays up for days).
            var today = DateTime.Now.ToString("yyyyMMdd");
            if (today != _currentLogDay)
            {
