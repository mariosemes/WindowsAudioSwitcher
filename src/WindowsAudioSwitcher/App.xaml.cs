...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        try
        {
            ...
            if (!createdNew)
            {
                ...
                // We don't own this mutex (another instance does), so just release our
                // handle and null it out — otherwise OnExit's ReleaseMutex would throw
                // (this thread isn't the owner) and we'd leak the handle until exit.
    ...
    private async Task CheckForUpdatesAsync()
    {
        ...
        // can safely touch WPF state. BeginInvoke (not Invoke) so we don't block this
        // background continuation waiting on the UI thread — there's nothing to wait for.
        _ = Dispatcher.BeginInvoke(() => UpdateAvailable?.Invoke(info));
    }
    ...
    /// <summary>
    /// Persists settings WITHOUT re-running the rule engine. Used to flush a
    /// pending debounced edit on app exit: we want the user's last change saved,
    /// but switching the default device as a side effect of shutting down would
    /// be surprising.
    /// </summary>
    public void SaveSettingsOnly(AppSettings updated)
    {
        _settings = updated;
        try { SettingsStore.Save(_settings); }
        catch (Exception ex) { Logger.Error("SettingsStore.Save (exit flush) failed", ex); }
        try { StartupRegistration.Sync(_settings.RunOnStartup); }
        catch (Exception ex) { Logger.Error("StartupRegistration.Sync failed", ex); }
    }
    ...
    protected override void OnExit(ExitEventArgs e)
    {
        ...
        // Flush any debounced edit the user made in the last ~500 ms before the
        // settings window's timer got a chance to tick — otherwise that change is
