...
public sealed class AudioDeviceManager : IDisposable, IMMNotificationClient
{
    ...
    public IReadOnlyList<AudioDevice> GetDevices(DataFlow flow)
    {
        ...
        try
        {
            ...
            {
                // MMDevice is IDisposable and wraps COM handles (an IMMDevice plus a
                // lazily-opened IPropertyStore once FriendlyName is read). Read the
                // fields we need into a plain record, then dispose the device so the
                // native handles are released now instead of at the next GC. This runs
                // on every device-change event, so deterministic disposal matters.
                using (d)
                {
    ...
    public DeviceSnapshot Snapshot(DataFlow flow)
    {
        ...
        try
        {
            foreach (var d in _enumerator.EnumerateAudioEndPoints(flow, DeviceState.Active))
            {
                // Dispose each MMDevice after copying its fields — see GetDevices.
                using (d)
                {
                    list.Add(new AudioDevice(d.ID, d.FriendlyName, flow, d.ID == defaultId));
                }
