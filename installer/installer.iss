...
[Run]
; Interactive installs: offer a "Launch app" checkbox on the Finished page. Launched
; without --tray so a first-time user sees the settings window. Skipped when silent.
Filename: "{app}\{#AppExeName}"; Description: "Launch {#AppName}"; \
    Flags: nowait postinstall skipifsilent
; Silent installs (the in-app auto-updater's relaunch path): come back minimized to
; the tray with --tray instead of popping the window open. Runs only when silent, so
; it never double-launches alongside the interactive entry above.
